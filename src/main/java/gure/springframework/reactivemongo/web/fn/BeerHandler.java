...
public class BeerHandler {
    ...
    public Mono<ServerResponse> getBeersByPriceRange(ServerRequest request) {

        Optional<String> minParam = request.queryParam("min");
        Optional<String> maxParam = request.queryParam("max");

        if (minParam.isPresent() && maxParam.isPresent()) {
            BigDecimal min = new BigDecimal(minParam.get());
            BigDecimal max = new BigDecimal(maxParam.get());
            Flux<BeerDTO> flux = beerService.findAllByPriceBetween(min, max);
            return ServerResponse.ok().body(fromPublisher(flux, BeerDTO.class));
        } else {
            return Mono.error(new ResponseStatusException(HttpStatus.BAD_REQUEST, "Both 'min' and 'max' query parameters are required."));
        }
    }
}
